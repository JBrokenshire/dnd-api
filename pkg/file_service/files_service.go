...
func (s *FilesService) AllowedUserFileAccess(enterpriseUid string) map[string]FilesAccess {
	paths := make(map[string]FilesAccess)

	// Give READ ONLY access to the public folder
	paths["public/*"] = FilesAccessRead

	// MDM users should have RW access to all files in their enterprise
	paths[fmt.Sprintf("enterprises/%v/files/*", enterpriseUid)] = FilesAccessReadWrite
