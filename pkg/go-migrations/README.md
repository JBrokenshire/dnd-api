...
```
    bash $GOPATH/purplevisits.com/mdm/pkg/go-migrations/init.sh
    or if you use vendor folder
    bash vendor/purplevisits.com/mdm/pkg/go-migrations/init.sh
```

Or you can copy sources from your dependencies path manually if you have trouble with command.
For example from:
```
    .../dnd-api/pkg/go-migrations/template
```
