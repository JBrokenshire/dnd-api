...
import (
	"fmt"
	"github.com/JBrokenshire/dnd-api/pkg/go-migrations/builder/contract"
	"github.com/JBrokenshire/dnd-api/pkg/go-migrations/builder/mysql/column"
	"github.com/JBrokenshire/dnd-api/pkg/go-migrations/builder/mysql/info"
	"github.com/JBrokenshire/dnd-api/pkg/go-migrations/builder/mysql/key"
	"github.com/JBrokenshire/dnd-api/pkg/go-migrations/config"
...
func (t *Table) PrimaryKey(Column string) contract.Column {
	if ok, c := t.hasColumn(Column); ok {
		...
		return t.Integer("id").Autoincrement().Primary()
	}
}
...
func (t *Table) createTableSQL() string {
	...
	for _, c := range t.columns {
		...
		t.checkUniqueKey(c)
		if c.IsPrimary() {
			t.primaryKey = key.NewPrimaryKey(t.name, c.GetName()).GenerateKeyName()
		}
	}
	if t.timestamps {
...
func (t *Table) renameTableSQL() string {
	return fmt.Sprintf("RENAME TABLE %v TO %v;", t.name, t.newName)
}
